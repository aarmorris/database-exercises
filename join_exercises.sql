...
SELECT d.dept_name AS 'Department Name', CONCAT(e.first_name, ' ', e.last_name) AS 'Department Manager' FROM employees e
    JOIN dept_manager dm on e.emp_no = dm.emp_no
    JOIN departments d on dm.dept_no = d.dept_no WHERE dm.to_date LIKE '9%' AND e.gender = 'F' ORDER BY d.dept_name;
...
SELECT title, COUNT(title) FROM titles t
    ...
    JOIN dept_emp de on e.emp_no = de.emp_no WHERE de.dept_no LIKE '%9' AND de.to_date LIKE '9%' AND t.to_date LIKE '9%' GROUP BY title ORDER BY COUNT(title) DESC;
